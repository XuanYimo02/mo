...
class GoodsAllListSpider(DmSpider, AutoParse):
    name = 'alumniofny:goods_all_list'
    platform = "alumniofny"
    task_id = 'alumniofny'
    sch_task = 'alumniofny-task'
    today = datetime.now()
    ...
    def start_requests(self):
        ...
        """
        page = 1
        url = 'https://www.alumniofny.com/collections/all?page=1'
    ...
    async def parse_home(self, response, **kwargs):
        ...
        for productVariant in productVariants:
            url = 'https://www.alumniofny.com' + productVariant["product"]["url"]
            yield scrapy.Request(
                url=url,
    ...
    async def parse_detail(self, response, **kwargs):
        ...
        爬虫详情页抓取
        """
        pattern = r'var afterpay_product = (.*?});'
        data_start = response.text.replace('\n', '')
        result = re.search(pattern, data_start)
        json_data = json.loads(result.group(1))
...
if __name__ == '__main__':
    ...
    execute('scrapy crawl alumniofny:goods_all_list'.split(' '))
