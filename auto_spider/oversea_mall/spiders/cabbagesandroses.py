...
class GoodsAllListSpider(DmSpider, AutoParse):
    name = 'cabbagesandroses:goods_all_list'
    platform = "cabbagesandroses"
    task_id = 'cabbagesandroses'
    sch_task = 'cabbagesandroses-task'
    today = datetime.now()
    ...
    def start_requests(self):
        ...
        page = 1
        urls = [
            'https://www.cabbagesandroses.com/collections/womenswear?page=1',
            'https://www.cabbagesandroses.com/collections/new-in?page=1',
            'https://www.cabbagesandroses.com/collections/ss24?page=1',
            'https://www.cabbagesandroses.com/collections/c-r-interiors-collection?page=1',
            'https://www.cabbagesandroses.com/collections/sale?page=1',
        ]
        for url in urls:
    ...
    async def parse_home(self, response, **kwargs):
        ...
        artical_list = response.xpath('//*[@id="content"]/div[3]/article')
        for artical in artical_list:
            url = 'https://www.cabbagesandroses.com' + artical.xpath('./div[1]/a/@href').extract()[0]
            yield scrapy.Request(
                url=url,
    ...
    async def parse_list(self, response, **kwargs):
        ...
        爬虫列表页抓取
        """
        a_list = response.xpath('//*[@class="main__wrapper"]/div[2]/section//div/div[4]/div/a')
        for a in a_list:
            url = 'https://www.cabbagesandroses.com' + a.xpath('./@href').extract()[0]
            yield scrapy.Request(
                url=url,
    ...
    async def parse_detail(self, response, **kwargs):
        ...
        爬虫详情页抓取
        """
        pattern = r'window.SwymProductInfo.product = (.*?});'
...
if __name__ == '__main__':
    ...
    execute('scrapy crawl cabbagesandroses:goods_all_list'.split(' '))
