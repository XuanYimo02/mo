...
class GoodsAllListSpider(DmSpider, AutoParse):
    name = 'petitestudionyc:goods_all_list'
    platform = "petitestudionyc"
    task_id = 'petitestudionyc'
    sch_task = 'petitestudionyc-task'
    today = datetime.now()
    ...
    def start_requests(self):
        ...
        爬虫首页
        """
        url = 'https://www.petitestudionyc.com/collections/shop-all?page=1'
    ...
    async def parse_home(self, response, **kwargs):
        ...
        meta = response.meta
        page = meta['page']
        div_list = response.xpath('//*[@id="main-collection-product-grid"]/div')
        for div in div_list:
            url = 'https://www.petitestudionyc.com' + div.xpath('./div/div[2]/a/@href').extract()[0]
            yield scrapy.Request(
                url=url,
        ...
        if len(div_list) > 1:
            page += 1
            url = f'https://www.petitestudionyc.com/collections/shop-all?page={page}'
            yield scrapy.Request(
                url=url,
                callback=self.parse_home,
                meta={'page': page}
    ...
    # def dis_callback(self, response):
    #     data = json.loads(response.body)
    #     logger.info(data)
    #     if data.get('code') != defaults.DM_SCHEDULER_API_SUCCESS_CODE:
    #         self.logger.warning("post data with error: %s", data.get('msg'))
    #     else:
    #         if data.get('data', {}).get('errorSize', 0) > 0:
    #             self.logger.warning("post data with error: %s", data.get('data', {}).get('errorResult'))
    #         if response.meta.get(defaults.DM_SCHEDULER_TASK_ID) and response.meta.get(
    #                 defaults.DM_SCHEDULER_FORCE_TASK):
    #             data = json.loads(response.body)
    #             if data.get('code') == defaults.DM_SCHEDULER_API_SUCCESS_CODE and data.get('data', {}).get(
    #                     'errorSize', 0) == 0:
    #                 self.success_callback(response.meta.get(defaults.DM_SCHEDULER_TASK_ID))
    #             else:
...
if __name__ == '__main__':
    ...
    execute('scrapy crawl petitestudionyc:goods_all_list'.split(' '))
