...
class GoodsAllListSpider(DmSpider, AutoParse):
    name = 'allbirds:goods_all_list'
    platform = "allbirds"
    task_id = 'allbirds'
    sch_task = 'allbirds-task'
    today = datetime.now()
    ...
    async def parse_home(self, response, **kwargs):
        ...
        爬虫首页抓取
        """
        pattern = r'masterProducts":(.*?]),'
        data_start = response.text.replace('\n', '')
        result = re.search(pattern, data_start)
        gid_list = json.loads(result.group(1).replace('\n', ''))
        urls = []
        for gid in gid_list:
            url = f'https://www.allbirds.com/collections/{gid}?view=json'
            urls.append(url)
    ...
    async def parse_list(self, response, **kwargs):
        ...
        if products != {}:
            for key, product in products.items():
                ...
                url = 'https://www.allbirds.com/products/' + handle
                yield scrapy.Request(
                    url=url,
...
if __name__ == '__main__':
    ...
    execute('scrapy crawl allbirds:goods_all_list'.split(' '))
