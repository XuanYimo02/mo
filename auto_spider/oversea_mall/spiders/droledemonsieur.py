...
class GoodsAllListSpider(DmSpider, AutoParse):
    num = 0
    number = 0
    name = 'droledemonsieur:goods_all_list'
    platform = "droledemonsieur"
    task_id = 'droledemonsieur'
    sch_task = 'droledemonsieur-task'
    today = datetime.now()
    ...
    def start_requests(self):
        ...
        爬虫首页
        """
        url = 'https://droledemonsieur.com/collections/see-all'
    ...
    async def parse_home(self, response, **kwargs):
        ...
        爬虫首页抓取
        """
        div_list = response.xpath('//*[@id="shopify-section-collection"]/div[1]/div/div/div/div')
        for div in div_list:
            try:
                url = 'https://droledemonsieur.com' + div.xpath('./div/div[4]/a/@href').extract()[0]
                yield scrapy.Request(
                    url=url,
    ...
    async def parse_detail(self, response, **kwargs):
        ...
        try:
            pattern = rf'productConfig\[\"{item}\"\] = (.*?);		    	    productConfig'
            result = re.search(pattern, data_start)
            json_data = json.loads(result.group(1))
        except:
            pattern = rf'productConfig\[\"{item}\"\] = (.*?);		var shipping ='
            result = re.search(pattern, data_start)
            json_data = json.loads(result.group(1))
...
if __name__ == '__main__':
    ...
    execute('scrapy crawl droledemonsieur:goods_all_list'.split(' '))
